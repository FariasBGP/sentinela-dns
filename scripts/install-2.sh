...
# Adiciona python3-requests para o novo Agente Sentinela
...
EOF
  ...
  # Copia TODOS os dashboards .json do repositório
  inf "A copiar dashboards do repositório..."
  cp -f "${REPO_ROOT}/grafana/provisioning/dashboards/"*.json \
    /var/lib/grafana/dashboards/unbound/ 2>/dev/null || warn "Nenhum ficheiro .json de dashboard encontrado."
...
# Copia e ativa as unidades do systemd (service e timer) para o Coletor NXDOMAIN
if [[ -d "${REPO_ROOT}/systemd" ]] && [[ -f "${REPO_ROOT}/systemd/top-nxdomain.service" ]]; then
  ...
  warn "Diretório 'systemd' ou ficheiro 'top-nxdomain.service' não encontrado. Pulei a instalação do coletor NXDOMAIN."
fi

inf "Instalando o Agente Sentinela..."

# Cria a pasta de configuração do agente (o ficheiro agent.conf deve ser criado manualmente)
install -d -m 0700 /etc/sentinela

# Copia o script do agente
if [[ -f "${REPO_ROOT}/scripts/sentinela-agent.py" ]]; then
    ...
    warn "Script sentinela-agent.py não encontrado no repositório."
fi

# Copia e ativa o serviço do agente
if [[ -d "${REPO_ROOT}/systemd" ]] && [[ -f "${REPO_ROOT}/systemd/sentinela-agent.service" ]]; then
    install -m 0644 "${REPO_ROOT}/systemd/sentinela-agent.service" /etc/systemd/system/
    systemctl daemon-reload
    systemctl enable --now sentinela-agent.service
    ok "Serviço do Agente Sentinela está ativo e a correr."
else
    warn "Diretório 'systemd' ou ficheiro 'sentinela-agent.service' não encontrado. Pulei a instalação do Agente."
fi

# ===== Health-check final =====
inf "Verificando saúde final de todos os serviços..."
# Adiciona os novos serviços ao loop de verificação
for s in unbound unbound_exporter prometheus grafana-server prometheus-node-exporter top-nxdomain.timer sentinela-agent.service; do
  if [[ "$s" == *.timer ]]; then
    systemctl is-active --quiet "$s" && ok "Timer ativo: $s" || warn "Timer INATIVO: $s"
  else
    # Verifica se o serviço deve ser instalado antes de alertar sobre inatividade
    SHOULD_INSTALL=yes
    if [[ "$s" == "prometheus" ]] && [[ "$PROM_INSTALL" != "yes" ]]; then SHOULD_INSTALL=no; fi
    if [[ "$s" == "grafana-server" ]] && [[ "$GRAFANA_INSTALL" != "yes" ]]; then SHOULD_INSTALL=no; fi
    if [[ "$s" == "prometheus-node-exporter" ]] && [[ "$NODE_EXPORTER_INSTALL" != "yes" ]]; then SHOULD_INSTALL=no; fi
    
    if systemctl is-active --quiet "$s"; then
      ok "Serviço/Timer ativo: $s"
    elif [[ "$SHOULD_INSTALL" == "no" ]]; then
      inf "Serviço não instalado (ignorado): $s"
    else
      warn "Serviço/Timer INATIVO (ok se não instalado): $s"
    fi
  fi
done
